...
d = '2109_2';
dropbox ='MSE_hinge';
outputpath ='/research2/ECCV_journal/with_light/NIR_single_MSE_ang_hinge';
suboutputpath = sprintf('output/%s/save',d);
%suboutputpath ='output/2109/save';
normalpath = '/research2/IR_normal_small/save';
maskpath = '/research2/IR_normal_small/mask';
fileID = fopen(sprintf('~/Dropbox/ECCV_result/light_fixed/with_light/nodetail_MSE_hinge%s_%d.csv',d),'w');
fprintf(fileID,'mean_error,mean_angle_error,10deg\n');
epochs = dir(fullfile(outputpath,sprintf('output/%s/save011',d),'1','*.png'));
%epochs = dir(fullfile(outputpath,'output/2109/save011','1','*.png'));
for e =1:length(epochs)
    ...
    for i = 1: p %object
        for j=1:pp % tilt
            ...
            im2 = im2double(imread(fullfile(outputpath,sprintf('%s%s',suboutputpath,folder),int2str(j),epochs(e).name)));
